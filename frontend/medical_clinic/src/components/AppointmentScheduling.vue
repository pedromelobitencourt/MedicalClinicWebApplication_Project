    ...
    <div class="auth-inner">
      ...
      <form @submit.prevent="efetuarAgendamento">
        <div class="form-especialidade">
          <label for="EspecialidadeSelecionada"
        ...
        <div class="form-dataConsulta" style="margin-top: 20px">
          ...
          <b-form-datepicker
            ...
            date-format="d/m/Y"
            @input="atualizarHorariosDisponiveis"
            :disabled="!this.MedicoSelecionado"
          >
          </b-form-datepicker>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      dataC: "",
      HorariosDisponiveis: [],
      ListaPadrao: ["08", "09", "10", "11", "12", "13", "14", "15", "16", "17"],
      TelefoneP: "",
      NomeP: "",
      EmailP: "",
  ...
  methods: {
      ...
      await axios
        ...
        .then((res) => {
          const especialidadesSet = new Set(
            res.data.map((med) => med.especialidade)
          );

          this.especialidades = Array.from(especialidadesSet);
          console.log(
            "carregarMedicos, Lista de especialidades:",
            this.especialidades
          );
        })
        .catch((error) => {
    ...
    async selecionarMedicos(event) {
      ...
      if (
        ...
        await axios
          .get(
            `http://localhost:8000/medicosname/${this.EspecialidadeSelecionada}`
          )
          .then((response) => {
            this.medicos = response.data;
            console.log(
              "Selecionar medicos, lista de medicos selecionados",
              this.medicos
            );
          })
          .catch((error) => {
            console.error(
              "Erro na requisição ao backend (controllerMedicos):",
              error
            );
          });
      }
    ...
    async selecionarDataEHorario(event) {
      ...
      if (this.MedicoSelecionado !== "" && this.MedicoSelecionado !== null) {
        await axios
          .get(`http://localhost:8000/agenda/${this.MedicoSelecionado}`)
          .then((response) => {
            ...
            for (const obj of this.Datas) {
              ...
              for (const obj2 of this.Datas) {
                if (chave === obj2.data.split("T")[0].slice(0, 10)) {
                  ...
                  this.DatasEHorariosOcupados[chave].push(
                    obj2.data.split("T")[1].slice(0, 2)
                  );
                }
              }
            }
            console.log(
              "Dicionario DatasEHorariosOcupados:",
              this.DatasEHorariosOcupados
            );
          })
          .catch((error) => {
            console.error(
              "Erro na requisição ao backend (controllerAgenda):",
              error
            );
          });
      }
    ...
    atualizarHorariosDisponiveis() {
      ...
      for (const chave in this.DatasEHorariosOcupados) {
        ...
        if (chave === this.dataC) {
          console.log(
            "lista escolhida, lista padrao:",
            lista,
            this.ListaPadrao
          );

          this.HorariosDisponiveis = this.ListaPadrao.filter(
            (itemB) => !lista.includes(itemB)
          );

          console.log(
            "HorariosDisponiveis,Horarios disponiveis:",
    ...
    async efetuarAgendamento() {
      ...
      try {
        const dataCompletaString =
          this.dataC + " " + this.HorarioSelecionado + ":00:00";
        ...
        console.log(
          ...
          this.EmailP,
          this.TelefoneP,
          parseInt(this.MedicoSelecionado)
        );
